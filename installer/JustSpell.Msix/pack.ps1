...
dotnet publish (Join-Path $root "src\JustSpell\JustSpell.csproj") `
...
$msix = Join-Path $outDir "JustSpell.msix"
if (Test-Path $msix) { Remove-Item $msix -Force }
& $makeappx pack /d $layout /p $msix /o
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$appinstaller = Join-Path $outDir "JustSpell.appinstaller"
...
<AppInstaller Version="$display" xmlns="http://schemas.microsoft.com/appx/appinstaller/2018">
  <MainPackage Name="JustSpell" Publisher="CN=JustSpell" Version="$display"
