...
public partial class MainWindow : Window
{
    ...
    {
        var installed = _installed is null ? "PuffinProof is not installed." : "Installed version: " + _installed + ".";
        if (string.IsNullOrWhiteSpace(_feed.GithubRepo))
        {
            return installed +
                   " No GitHub repo in feed.json (or PUFFINPROOF_GITHUB_REPO). You can still install a local MSIX.";
        }
    ...
    private async Task InstallLatestAsync()
    {
        ...
        var msix = await GitHubRelease.DownloadAsync(_latest.DownloadUrl, progress, CancellationToken.None)
            .ConfigureAwait(true);
        LaunchMsix(msix);
        Status.Text = "Windows is installing the MSIX. PuffinProof does not need administrator rights.";
    }
    ...
    private void OnLocalMsi(object sender, RoutedEventArgs e)
    {
        var dialog = new OpenFileDialog
        {
            ...
            Title = "Choose a PuffinProof package"
        };
        var local = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "dist", "PuffinProof.msix"));
