...
using PuffinProof.UI;

namespace PuffinProof;
...
public partial class App : Application
{
    ...
    private void OnStartup(object sender, StartupEventArgs e)
    {
        ...
        {
            MessageBox.Show(
                "PuffinProof does not use administrator rights. Close this window and start it normally — not “Run as administrator.”",
                "PuffinProof",
                MessageBoxButton.OK,
        ...
        _mutex = new Mutex(true, @"Local\PuffinProof.SingleInstance", out var created);
        if (!created)
        {
            MessageBox.Show("PuffinProof is already running. Look for it in the system tray.", "PuffinProof");
            Shutdown();
            return;
        }
        ...
        {
            MessageBox.Show(
                "PuffinProof could not load its dictionary.\n\n" + ex.Message,
                "PuffinProof",
                MessageBoxButton.OK,
    ...
    private void PauseInCurrentApp()
    {
        ...
        if (string.IsNullOrWhiteSpace(name) ||
            string.Equals(name, "PuffinProof", StringComparison.OrdinalIgnoreCase))
        {
            _tray?.ShowBalloon("PuffinProof", "Type in the app you want to skip, then try again.");
            return;
        }
        ...
        _tray?.ShowBalloon("PuffinProof", $"Paused in {name}. You can undo this in Settings.");
    }
    ...
    private void RememberForegroundApp()
    {
        ...
        if (string.Equals(name, "PuffinProof", StringComparison.OrdinalIgnoreCase) ||
