...
public class SpellEngineTests
{
    ...
    {
        var tempUser = Path.Combine(Path.GetTempPath(), "justspell-tests-" + Guid.NewGuid().ToString("n") + ".txt");
    ...
    {
        var engine = CreateEngine();
        engine.AddToUserDictionary("JustSpell");
        Assert.True(engine.IsCorrect("JustSpell"));
        Assert.Equal(SpellDecision.Correct, engine.Evaluate("JustSpell", new AppSettings()));
    }
